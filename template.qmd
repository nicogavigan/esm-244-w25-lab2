...
m.2 <- lm(f2, penguins_clean)

m.3 <- lm(f3, penguins_clean)
...
# the sample size in the first function is too large because the sample only has 5 variables. replace=TRUE allows the already sampled values to be resampled
...
for(i in 1:folds){
  ...
  # Run for one model
  kfold_train_m.1 <- lm(f1, kfold_train_df)
  
  #Get the predictions from the model
  predict_test_kfold_m.1 <- kfold_test_df |>
    mutate(model1 = predict(kfold_train_m.1, kfold_test_df))
  
  # Summarize/calculate the rmse for that model
    kfold_rmse <- predict_test_kfold_m.1 |>
    summarize(rmse_m.1 = calc_rmse(mass, model1))
  
  rmse_vec[i]<-kfold_rmse$rmse_m.1
}
...
```{r}
final_mod<-lm(f2, penguins_clean)
